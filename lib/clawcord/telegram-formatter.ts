...
export function formatHelpMessage(): string {
  return [
    ...
    ``,
    `<b>🔍 Token Analysis:</b>`,
    `/whale &lt;mint&gt; — Whale activity`,
    `/holders &lt;mint&gt; — Holder distribution`,
    `/momentum &lt;mint&gt; — Price/volume signals`,
    `/risk &lt;mint&gt; — Risk breakdown`,
    ``,
    `<b> Info:</b>`,
    `/status — Bot status`,
    ``,
    ...
    `/setrisk &lt;1-10&gt; — Min score threshold`,
    `/autopost — Toggle auto-posting`,
    `/policy — Change filter policy`,
    `/mute · /unmute — Toggle signals`,
    ``,
    `<i>Tap any ticker in results to see detailed card with PFP, links &amp; CA</i>`,
  ].join("\n");
}
